...
class AssistModifier (object):
    """Applies changed assist settings to the active brush, with overrides.

    A single instance of this lives within the main `application.Application`
    instance. The AssistModifier has internal state how assist user drawing
    and FreehandMode look it,to apply assist method.
    """
    ...
    def _init_actions(self):
        self.action_group = self.app.builder.get_object("AssistModifierActions")

        # ToggleActions defined in resources.xml
        self.stabilizer_mode = self.app.find_action("AssistModeStabilizer")
        self.normal_mode = self.app.find_action("AssistModeNormal")

        # Each mode ToggleAction has a corresponding setting
        self.stabilizer_mode.setting_name = "stabilizer"
        self.normal_mode.setting_name = None

        # The set of toggle actions our fakery swicthes between
        self._toggle_actions = (self.stabilizer_mode, self.normal_mode)
   ...
   #            other_action.unblock_activate()
    ...
    def assist_mode_normal2_cb(self, action):
    ...
    def assist_mode_stabilizer2_cb(self, action):
