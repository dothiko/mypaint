...
public:
...
#endif
        if (targ->is_filled_with(PIXEL_INVALID)) {
    ...
    { 
        ...
        if (m_processed > 0) {
            // A tile which marked `dirty`
            // should contains FLAG_WORKed pixel.
            // It is automatically removed by
            // KernelWorker::finalize() method.
            targ->set_dirty();
            m_processed = 0;
        }
    }
...
public:
    ...
    // This kernel never uses FLAG_WORK.
    ...
    virtual bool start(Flagtile * const targ, const int sx, const int sy) 
    {
        ...
        if(targ->is_filled_with(PIXEL_FILLED)
            ...
            //
            // And, essentially `filled with PIXEL_AREA tile` has
            // all same value, so when maximum level pixel is
            // different from PIXEL_AREA, it means entire that tile
            // should be `decided`.
            //
...
public:
    ...
    // This kernel does not start - step 
