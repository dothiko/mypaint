...
class Tilecache
{
    protected:
        ...
        PyObject *_generate_cache_key(const int tx, const int ty)
        {
        ...
        PyObject *_get_cached_tile(const int otx, 
                                   const int oty, 
        ...
        PyObject *_get_cached_tile_from_index(const int index, 
                                              const bool generate)
        {
        ...
        PyObject *_get_cached_key(const int index)
        {
        ...
        // to search pixels,but requested result is different 
        // for each operation.
        // For example, for dilation, we need 
...
class _GapCloser: public Tilecache<STATE_PIXEL, STATE_PIXEL> {
    ...
    protected:
        ...
        // inside some method.
        STATE_PIXEL _get_tile_info(const int index)
        {
#ifdef HEAVY_DEBUG
            assert(index >= 0);
        ...
        void _set_tile_info(PyArrayObject *tile, const STATE_PIXEL flag)
        {
#ifdef HEAVY_DEBUG
            assert(tile != NULL);
#endif
            STATE_PIXEL *pixel;
            STATE_PIXEL t_flag = flag;
        ...
        // This can easily follow cached tile when it internally generated
        // inside some method.
        void _set_tile_info(const int index, const STATE_PIXEL flag)
        {
#ifdef HEAVY_DEBUG
            assert(index >= 0);
            assert(index < MAX_CACHE_COUNT);
        ...
        void _dilate_contour(const int gap_radius) 
        {         
#ifdef HEAVY_DEBUG
                 ...
                 y++) 
            {
                     ...
                     x++) 
                {
                    ...
                    if (pixel != NULL   
                        ...
                        // If pixel is NULL, the tile is also NULL.
                        // so generate it, by passing true to 
                        // get_cached_pixel().
                        if (pixel == NULL)
        ...
        PyObject *_convert_state_tile(PyObject *py_state_dict, 
                                      PyArrayObject *py_surf_tile, 
        ...
        // Before call this method, init_cached_tiles() must be already 
        // called.
        // This method converts color tiles around (tx, ty) into 
        // state flag tiles.
        // also, this setup function sets newly generated 
        // state tile into cache.
        void _setup_state_tiles(PyObject *py_state_dict, 
                                PyObject *py_surfdict, 
                                const fix15_short_t *targ_pixel,
                                const fix15_t tolerance)
        {
            // py_surfdict is surface tile dictionary, 
            // It is not cached in this class.
            ...
            for (int i=0; i < 9; i++) {
                ...
                if (m_cache_tiles[i] == NULL) {
                    ...
                    if (surf_tile != NULL) {
                        ...
                        // The surf_tile might not be tile, 
                        // but _Tile wrapper object.
                        ...
                        if (is_tile_obj != 0) {
                            ...
                            surf_tile = PyObject_GetAttrString(
                                            surf_tile,
                                            ATTR_NAME_RGBA
                                            );
#ifdef HEAVY_DEBUG            
    ...
    public:
        ...
        void close_gap(PyObject *py_state_dict, // 8bit state tiles dict
                       ...
                       const int tx, const int ty, // current tile location
                       const fix15_short_t *targ_pixel,
                       const fix15_t tolerance,
    ...
    const int gap_radius) 
{
    ...
    fix15_short_t targ_pixel[4] = {
                                   (fix15_short_t)targ_r,
    ...
    const int dilation_size)    
{
    ...
    fix15_short_t fill_pixel[3] = {
                                   (fix15_short_clamp)(fill_r * alpha),
